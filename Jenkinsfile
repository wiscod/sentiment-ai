    ...
    stages {
        ...
        stage('Smoke Test') {
            ...
            steps {
                ...
                curl -sg "http://prometheus:9090/api/v1/query?query=up{job='sentiment-ai'}" | \\
                  grep -q '"value":.*1' || (curl -sg "http://prometheus:9090/api/v1/query?query=up{job='sentiment-ai'}" && exit 1)
